...
$active = fn(string $prefix) => str_starts_with($current, $prefix) ? 'active' : '';
@endphp
...
<a href="{{ route('kurikulum.ai.index') }}"
    class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium {{ $active('kurikulum.ai') }} bg-gradient-to-r from-purple-500/10 to-indigo-500/10 hover:from-purple-500/20 hover:to-indigo-500/20"
    style="color: var(--text-secondary);">
...
<a href="{{ route('guru.ai.index') }}"
    class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium {{ $active('guru.ai') }} bg-gradient-to-r from-purple-500/10 to-indigo-500/10"
    style="color: var(--text-secondary);">
    <svg class="w-[18px] h-[18px] flex-shrink-0 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
    </svg>
...
['route'=>'admin.kategori-mata-pelajaran.index', 'prefix'=>'admin.kategori-mata-pelajaran', 'label'=>'Kategori Mapel', 'icon'=>'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10'],
['route'=>'admin.kkm.index', 'prefix'=>'admin.kkm', 'label'=>'KKM per Tingkatan', 'icon'=>'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
