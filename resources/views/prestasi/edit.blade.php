...
{{-- ============================================================ --}}
<x-app-layout>
    <x-slot name="header">Edit Prestasi</x-slot>
    ...
    <div class="mb-6 flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-semibold text-siakad-dark">
                Edit — {{ $prestasi->nama_prestasi }}
            </h2>
            <p class="text-sm text-siakad-secondary mt-0.5">
                {{ $prestasi->santri->nama_lengkap }}
            </p>
        </div>
    </div>

    <form method="POST" action="{{ route('kesantrian.prestasi.update', $prestasi) }}">
        @csrf @method('PUT')
        @include('prestasi._form', ['submitLabel' => 'Perbarui Prestasi'])
    </form>
</x-app-layout>