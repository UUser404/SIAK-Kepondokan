...
<x-app-layout>
    <x-slot name="header">Ekstrakurikuler</x-slot>

    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-xl font-semibold text-siakad-dark dark:text-white">Ekstrakurikuler</h2>
            <p class="text-sm text-siakad-secondary dark:text-gray-400 mt-0.5">
                Kelola daftar ekstrakurikuler yang tersedia (dipakai wali kelas untuk input nilai)
            </p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm table-saas">
                <thead style="background-color: rgba(35,76,106,0.04);">
                    <tr>
                        @foreach(['Nama Ekstrakurikuler','Pembina','Status','Aksi'] as $h)
                    ...
                    <tr class="dark:hover:bg-gray-700/30">
                        ...
                        <td class="px-5 py-3.5">
                            <div class="flex items-center gap-1">
                                <a href="{{ route('admin.ekstrakurikuler.show', $e) }}"
                                ...
                                <a href="{{ route('admin.ekstrakurikuler.edit', $e) }}"
                                    class="p-2 rounded-lg text-siakad-secondary dark:text-gray-400
                                      hover:bg-siakad-primary/10 hover:text-siakad-primary transition">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <form method="POST" action="{{ route('admin.ekstrakurikuler.destroy', $e) }}"
                                    onsubmit="return confirm('Nonaktifkan {{ addslashes($e->nama) }}?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                        class="p-2 rounded-lg text-siakad-secondary dark:text-gray-400
