...
function addMessage(role, content, time) {
    ...
    const bubble = isUser
        ...
        : `<div class="inline-block px-4 py-3 rounded-2xl rounded-tl-sm text-sm bg-gray-100 text-siakad-dark max-w-[80%]">${formatted}</div>`;
...
async function sendMessage() {
    ...
    try {
        const res = await fetch(CHAT_ROUTE, {
            ...
            body: JSON.stringify({ message, history: chatHistory.slice(-10), santri_id: santriId }),
        });
