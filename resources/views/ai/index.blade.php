...
<x-app-layout>
    ...
    {{-- lg:h-[calc(100vh-8rem)] -- 8rem = topbar h-16 (4rem) + padding
         <main> md:p-8 atas+bawah (2rem+2rem). Cuma diterapkan mulai lg
         (desktop) -- di mobile SENGAJA dibiarkan scroll natural dulu,
         belum dibahas/disepakati layout mobile-nya. --}}
    <div class="lg:h-[calc(100vh-8rem)] lg:flex lg:flex-col">

        <div class="mb-6 lg:mb-4 lg:flex-shrink-0 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
        ...
        <div class="grid lg:grid-cols-3 gap-5 lg:flex-1 lg:min-h-0">

            {{-- Chat area --}}
            <div class="lg:col-span-2 flex flex-col h-[70vh] lg:h-full">
            ...
            <div class="space-y-4 lg:h-full lg:overflow-y-auto lg:pr-1">
                ...
                <div class="card-saas p-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="p-2.5 rounded-xl bg-gray-50 border border-transparent cursor-pointer
                            transition hover:bg-purple-50 hover:border-purple-200"
                            onclick="loadRiwayat(this)"
    ...
    <script>
        let chatHistory = [];
        // $chatRoute dihitung di controller (AiAdvisorController::index(),
        // lewat peta ROLE_ROUTE_PREFIX) -- BUKAN ditebak di sini lagi.
        // Sebelumnya cuma cek role==='guru' vs fallback ke kurikulum.ai.chat,
        // jadi role kesantrian/admin/sysadmin salah arah / 404 kalau kirim
        // chat. Kalau nambah role baru yang boleh akses AI Advisor, ubahnya
        // di ROLE_ROUTE_PREFIX controller, BUKAN di sini.
        const CHAT_ROUTE = '{{ $chatRoute }}';
        ...
        /**
         * Muat 1 riwayat percakapan (pertanyaan+jawaban) yang diklik dari
         * sidebar ke jendela chat -- data pertanyaan/jawabannya sudah ada
         * di halaman lewat data-question/data-answer (embed dari
         * AiConversationLog di server, tidak perlu fetch ulang ke server).
         * Chat yang sedang berjalan (kalau ada) DIGANTI, bukan ditambah --
         * dikonfirmasi dulu kalau ada percakapan aktif supaya tidak ke-
         * hapus tidak sengaja.
         */
        function loadRiwayat(el) {
            if (chatHistory.length > 0 && !confirm('Muat percakapan ini? Percakapan yang sedang berlangsung di jendela chat akan diganti.')) {
                return;
            }
            ...
            addMessage('user', question, time);
            chatHistory.push({
                role: 'user',
                content: question
            });

            addMessage('assistant', answer, time);
            chatHistory.push({
                role: 'assistant',
                content: answer
            });
        ...
        function addMessage(role, content, time) {
            ...
            // "flex-1" di wrapper konten melebar ikut sisa ruang chat, tapi
            // bubble di dalamnya "inline-block" -- tanpa text-align di parent,
            // dia nempel di ujung KIRI wrapper (bukan deket ke avatar yang ada
            // di kanan). flex-row-reverse cuma balik urutan avatar/konten,
            // TIDAK ngatur perataan isi di dalam wrapper konten itu sendiri.
            div.innerHTML = `${avatar}<div class="flex-1 ${isUser ? 'text-right' : ''}">${bubble}${label}</div>`;
