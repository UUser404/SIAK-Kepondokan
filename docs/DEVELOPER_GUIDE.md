...
33. **Fitur baru: Import Santri Baru (bulk CREATE, bukan update) -- khusus sysadmin**:
    - **Kebutuhan**: migrasi data awal 300-1000 santri dari catatan lama (Excel), berbeda dari fitur "Import Kelas & Asrama Massal" yang sudah ada -- fitur itu cuma meng-UPDATE kelas/asrama santri yang SUDAH TERDAFTAR (dicocokkan by NISN), tidak bisa dipakai untuk membuat santri baru dari nol.
    - **Kolom Excel yang dipakai** (5 kolom, semua wajib): NISN, Nama Lengkap, Kelas (rombel SPESIFIK seperti "7A", bukan level seperti "Kelas 7"), Angkatan, Jenis Kelamin (terima "L"/"P" atau "Laki-laki"/"Perempuan", case-insensitive).
    - **File baru**: `app/Imports/SantriBaruImport.php`, `app/Services/SantriCreateService.php` (beda dari `SantriImportService` yang sudah ada -- validasi lebih ketat, baris ber-error TIDAK BISA dipaksa "Setujui" karena approve di sini berarti benar-benar membuat record baru, bukan sekadar update field), `app/Exports/SantriCreateTemplateExport.php`, `resources/views/santri/import-baru.blade.php`, `resources/views/santri/import-baru-preview.blade.php`. Alur sama seperti fitur lama: upload -> preview (dengan tombol "Setujui Semua"/"Lewati Semua") -> konfirmasi simpan.
    - **Migration wajib**: `2026_09_05_000001_make_santri_nis_nullable.php` -- menutup gap dari perubahan NISN-jadi-acuan (poin 31): migration awal bikin kolom `nis` NOT NULL, padahal validasi Laravel sudah diubah jadi nullable. Tanpa migration ini, fitur Import Santri Baru (yang memang tidak menyertakan NIS sama sekali) gagal di level database (`SQLSTATE: nis cannot be null`), bukan cuma di fitur ini -- form Tambah/Edit Santri satuan tanpa isi NIS juga kena masalah yang sama.
    - **Keputusan akses**: dibatasi khusus **sysadmin**, TIDAK bisa diakses admin biasa (beda dari "Import Massal" yang tetap bisa admin biasa) -- karena ini operasi create massal yang lebih berisiko (data baru dalam jumlah besar) dibanding sekadar update kelas/asrama. Dua lapis: (a) `web.php`, 4 route `import-baru` dibungkus tambahan `Route::middleware('role:sysadmin')` di dalam grup `admin` yang sudah ada (menambah syarat, bukan menggantikan `role:admin|sysadmin` di level grup); (b) `index.blade.php`, tombolnya dibungkus `@role('sysadmin') ... @endrole`. Keduanya wajib jalan berbarengan -- `@role` di Blade cuma soal tampilan (sembunyikan tombol), yang benar-benar menahan akses langsung lewat URL adalah middleware di `web.php`.
    - **Resiko performa yang belum diuji**: tiap baris yang disetujui memicu 2 query insert (`Santri::create()` + `SantriKelas::create()`) dalam satu request HTTP -- untuk 1000 baris berarti ~2000 query sekaligus. Kemungkinan besar tetap cepat di lokal, tapi kalau ke depan terasa lambat atau kena `max_execution_time`, perlu diubah ke `chunk()` per-batch atau queue job -- belum dikerjakan.
